import { ApiProperty, ApiPropertyOptional } from '@nestjs/swagger';
import { IsNotEmpty, IsOptional, IsString, IsUUID, Matches, MaxLength } from 'class-validator';

export class SendWhatsAppDepositDocumentMessageRequestModel {
  @ApiProperty({ description: 'Business (tenant) ID — must match authenticated user' })
  @IsUUID()
  @IsNotEmpty()
  businessId: string;

  @ApiProperty({ example: '919XXXXXXXXX', description: 'Recipient WhatsApp number in international format without +' })
  @IsString()
  @Matches(/^\d{8,15}$/)
  to: string;

  @ApiProperty({
    description: 'Shop or business name used in the deposit receipt template body when outside the 24-hour window',
    example: 'Shree Jewellers',
  })
  @IsString()
  @IsNotEmpty()
  @Matches(/\S/)
  shopName: string;

  @ApiPropertyOptional({ description: 'Deposit account number shown in message history' })
  @IsOptional()
  @IsString()
  @MaxLength(64)
  depositNumber?: string;

  @ApiPropertyOptional({ description: 'Receipt number shown in message history' })
  @IsOptional()
  @IsString()
  @MaxLength(64)
  receiptNumber?: string;

  @ApiPropertyOptional({ description: 'Customer name shown in message history' })
  @IsOptional()
  @IsString()
  @MaxLength(120)
  customerName?: string;
}
