...
export function resolveTrialEndsAt(user: TrialUserFields, defaultTrialDays: number): Date {
  if (user.trialEndsAt) {
...
export function isTrialActive(user: TrialUserFields, defaultTrialDays: number, now = new Date()): boolean {
  const endsAt = resolveTrialEndsAt(user, defaultTrialDays);
  return endsAt != null && endsAt > now;
}

export function trialDaysRemaining(user: TrialUserFields, defaultTrialDays: number, now = new Date()): number {
  const endsAt = resolveTrialEndsAt(user, defaultTrialDays);
