...
export class PosSessionsController {
  ...
  @ApiParam({ name: 'id' })
  async getById(@Param('id') id: string, @Identity() identity: IIdentity): Promise<PosSessionResponseModel> {
  ...
  @ApiParam({ name: 'id' })
  async getQr(@Param('id') id: string, @Identity() identity: IIdentity): Promise<PosSessionQrResponseModel> {
